...
const FloatingContactBar = styled.div`
  ...
  background: rgba(${colors.background.secondary.replace('#', '')
    .match(/.{2}/g)
    ?.map(hex => parseInt(hex, 16))
    .join(', ')}, 0.6);
  padding: 12px;
  border-radius: 20px;
  border: 1px solid rgba(${colors.accent.primary.replace('#', '')
    .match(/.{2}/g)
    ?.map(hex => parseInt(hex, 16))
    .join(', ')}, 0.3);
...
const FloatingContactIcon = styled.a`
  ...
  &:hover {
    ...
    box-shadow: 0 4px 16px rgba(${colors.accent.primary.replace('#', '')
      .match(/.{2}/g)
      ?.map(hex => parseInt(hex, 16))
      .join(', ')}, 0.3);
